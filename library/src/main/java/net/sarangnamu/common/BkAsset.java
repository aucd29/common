...
public class BkAsset {
    private static final org.slf4j.Logger mLog = org.slf4j.LoggerFactory.getLogger(BkAsset.class);
        ...
        try {
            ...
            if (assets.length == 0) {
                ...
                if (!dir.exists() && !dir.mkdirs()) {
                    mLog.debug("could not create dir " + destPath);
                    return false;
                }
    ...
    public static void getAssetFileCountThread(final Context context, final String packageName, final String path, final FileCountListener l) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (l == null) {
                    mLog.error("IAssetFileCount is null");
                    return ;
                }
